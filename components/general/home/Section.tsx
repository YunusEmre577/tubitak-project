        ...
        {team.map(({ author, col_span, image, occ, desc }) => (
          <Dialog key={author}>
            <DialogTrigger asChild>
              <div
                className={cn(
                  "cursor-pointer border-accent relative overflow-hidden flex flex-col gap-2 justify-between border p-5 rounded-2xl",
                  {
                    "row-span-2": col_span === 2 && showDescription,
                    "w-full": !showDescription,
                  }
                )}
              >
                ...
                <footer className='justify-end flex items-center gap-3'>
                  ...
                  <Button color="secondary" isIconOnly><PlusIcon /></Button>
                </footer>
              </div>
            </DialogTrigger>
